...
The next pattern we see, and the next step for security, is to setup [API servers behind a firewall](https://getkong.org/docs/latest/network/) and only allow the Kong API Gateway be accessible to the public.
...
Application availability is important, and high availability can be achieved by configuring Kong in a multi-datacenter setup using [Apache Cassandra](https://getkong.org/about/faq/#apache-cassandra) as the backing datastore. In this configuration each geographically disparate datacenter is host to at least three Kong nodes, and the appropriate number of Cassandra nodes.
